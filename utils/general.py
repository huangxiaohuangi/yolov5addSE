...
def get_latest_run(search_dir='./runs222'):
    # Return path to most recent 'last.pt' in /runs222 (i.e. to --resume from)
...
def non_max_suppression(prediction, conf_thres=0.1, iou_thres=0.6, merge=False, classes=None, agnostic=False):
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        #     if torch.tensor(output[0][i][4]).item() > max_output :
        #         max_output = torch.tensor(output[0][i][4]).item()
        #         index = i
        ...
        # index = 0
        # for i in range(0, length):
        #     if torch.tensor(x[i][4]).item() > max_output:
        #         max_output = torch.tensor(x[i][4]).item()
        #         index = i
        # output = [x[[index]]]
...
def increment_dir(dir, comment=''):
    # Increments a directory runs222/exp1 --> runs222/exp2_comment
    n = 0  # number
...
def plot_evolution(yaml_file='runs222/evolve/hyp_evolved.yaml'):  # from utils.utils import *; plot_evolution()
